...
- Classifiers now list 3.13 and 3.14, the README badge reads "3.8 – 3.14", and the installation guide states what is actually tested. **No upper bound** is placed on `requires-python`: PIMMS uses no version-gated syntax or standard library API, so a new Python release is expected to work as soon as `numpy` / `scipy` / `mdtraj` publish wheels for it - that, rather than PIMMS itself, is the practical gate. (3.15 is not claimed: at the time of writing it is at 3.15.0a5 and `numpy` has no wheels for it, so nothing can be installed to test against.)
