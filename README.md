...
MOVE\_VMMC | **FLOAT** | Virtual-Move Monte Carlo (Whitelam & Geissler 2007) collective move. A seed chain is given a trial rigid translation and neighbouring chains are recruited into a moving cluster according to interaction-energy gradients (a neighbour joins when moving the seed alone would break their mutual attraction); the whole cluster then translates together. This lets correlated groups of chains move collectively and so escapes the kinetic traps that single-chain moves hit in dense / strongly-attractive (condensed) phases, while maintaining detailed balance. EXPERIMENTAL.
VMMC\_MAX_DISPLACEMENT | **INT** | Maximum magnitude (per dimension, lattice units) of the rigid translation proposed by a VMMC move. Small values give local collective moves (recommended; large values rarely clear hard-core clashes in dense phases). Defaults to 3.
VMMC\_MAX_CLUSTER | **INT** | Upper bound on the VMMC cluster size for the 1/n_c move-frequency correction (the cluster is rejected if it grows past the drawn cutoff). Clamped to the number of chains at runtime. Defaults to 1000.
