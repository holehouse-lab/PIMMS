...
def update_position(cnp.ndarray[DTYPE_t, ndim=1] old_position, cnp.ndarray[DTYPE_t, ndim=3] grid, DTYPE_t x_off, DTYPE_t y_off, DTYPE_t z_off, DTYPE_t XDIM, DTYPE_t YDIM, DTYPE_t ZDIM):
    
    cdef cnp.ndarray[DTYPE_t, ndim=1] new_position = np.zeros([3], dtype=int)
...
@cython.wraparound(False)
@cython.boundscheck(False)
def mega_crank(cnp.ndarray[DTYPE_t, ndim=3] grid, 
               cnp.ndarray[DTYPE_t, ndim=3] type_grid, 
               cnp.ndarray[DTYPE_t, ndim=2] idx_to_bead,
               ...
               float invtemp,
               int nsteps,
               cnp.ndarray[DTYPE_t, ndim=1] bead_selector,
               int passed_seed,
               int hardwall):
    """
    ...
    cdef cnp.ndarray[DTYPE_t, ndim=2] position_triptic = np.zeros([3, 3], dtype = int)    
    cdef cnp.ndarray[DTYPE_t, ndim=2] three_position_holder = np.zeros([3, 3], dtype = int)    
    cdef cnp.ndarray[DTYPE_t, ndim=2] two_position_holder = np.zeros([2, 3], dtype = int)    

    cdef cnp.ndarray[DTYPE_t, ndim=1] old_position = np.zeros([3], dtype = int)    
    cdef cnp.ndarray[DTYPE_t, ndim=1] anchor_bead = np.zeros([3], dtype = int)    
    cdef cnp.ndarray[DTYPE_t, ndim=1] new_position;
...
@cython.wraparound(False)
@cython.boundscheck(False)
cdef crank_it (cnp.ndarray[DTYPE_t, ndim=2] position_triptic, cnp.ndarray[DTYPE_t, ndim=3] grid, int XDIM, int YDIM, int ZDIM):
    """
...
#-----------------------------------------------------------------
# 
cdef crank_it_good (cnp.ndarray[DTYPE_t, ndim=2] position_triptic, cnp.ndarray[DTYPE_t, ndim=3] grid, int XDIM, int YDIM, int ZDIM):
    """
    Perform crankshaft move!
...
#-----------------------------------------------------------------
#
def single_bead_crank (cnp.ndarray[DTYPE_t, ndim=1] old_position, cnp.ndarray[DTYPE_t, ndim=3] grid, int XDIM, int YDIM, int ZDIM):
    """
    Perform crankshaft move!
...
@cython.wraparound(False)
@cython.boundscheck(False)
cdef long get_angle_energy_change(DTYPE_t bead_index,
                                   cnp.ndarray[DTYPE_t, ndim=2] idx_to_bead, 
                                   cnp.ndarray[DTYPE_t, ndim=1] new_position, 
                                   cnp.ndarray[long, ndim=7] angle_lookup):
    ...
    cdef cnp.ndarray[DTYPE_t, ndim=1] a = np.zeros([3], dtype=int)    
    cdef cnp.ndarray[DTYPE_t, ndim=1] b = np.zeros([3], dtype=int)    
    
    cdef long angle_penalty_new = 0;
    cdef long angle_penalty_old = 0;

    cdef cnp.ndarray[DTYPE_t, ndim=2] angle_positions  = np.zeros([5, 3], dtype=int)    
    cdef cnp.ndarray[DTYPE_t, ndim=1] intcode_lookup = np.zeros([5], dtype=int)    
    cdef int offset;
...
@cython.wraparound(False)
@cython.boundscheck(False)
cdef get_energy_change(cnp.ndarray[DTYPE_t, ndim=3] grid, 
                      cnp.ndarray[DTYPE_t, ndim=3] type_grid,
                      cnp.ndarray[DTYPE_t, ndim=1] old_position,
                      cnp.ndarray[DTYPE_t, ndim=1] new_position,
                      int LR_vs_SR,
                      cnp.ndarray[long, ndim=2] interaction_table, 
                      cnp.ndarray[long, ndim=2] LR_interaction_table,
...
@cython.wraparound(False)
@cython.boundscheck(False)
cdef int do_positions_stradle_pbc_boundary(cnp.ndarray[DTYPE_t, ndim=2] chain_positions, int chain_length):
    """                                       
