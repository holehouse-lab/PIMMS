...
KEYWORDS_DESCRIPTION = {
    ...
    'PARALLELIZE': ["bool", "Boolean flag (True/False) which, if set to True, runs the crankshaft (system_shake) move using the multi-threaded checkerboard kernel instead of the serial kernel. Works in both 2D and 3D. Beneficial for large, spatially dispersed systems; gives little benefit for small boxes (which decompose into a single block) or collapsed/dense single-droplet systems. The block decomposition is independent of the thread count, so results are identical for any number of threads; the parallel sampler targets the same equilibrium distribution but follows a different Markov chain than the serial run. Has no effect when any chain is frozen (it falls back to the serial kernel). Default = False."],
